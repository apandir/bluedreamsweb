  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled
          ? "py-2 bg-ocean-deep shadow-lg"
          : "py-4 bg-ocean-deep/95"
      }`}
    >
        ...
        <div className="hidden lg:flex items-center gap-0">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="font-body text-[10px] font-semibold tracking-[0.18em] uppercase text-primary-foreground/70 hover:text-gold px-3 py-2 transition-colors duration-300"
            >
              {link.label}
            </a>
          ))}
        ...
        <div className="bg-ocean-deep px-6 py-4 flex flex-col gap-1 border-t border-primary-foreground/10">
          {navLinks.map((link) => (
            <a
              ...
              className="font-body text-xs tracking-[0.15em] uppercase text-primary-foreground/70 hover:text-gold py-2.5 transition-colors"
            >
              {link.label}
            </a>
          ))}
